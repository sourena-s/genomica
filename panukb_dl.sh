...
for pop in "${populations[@]}"; do
	...
	if [ ! -d "$dl_path/$pop" ];then echo "making dir $dl_path/$pop"; mkdir -p "$dl_path/$pop";fi
        ...
        echo "Processing $pop: beta_col=$beta_col +1, se_col=$se_col +1 and var QC col $lowconf_opt"
...
parallel -j "$NPROC" --colsep ' ' process_url {} :::: <(awk -F$'\t' 'NR>1{print $11}' "$input_gwas_list" |awk -F$'\t' -v start_row=$start_row -v end_row=$end_row 'NR >= start_row && NR <= end_row {print $1}'|while read filename;do echo s3://pan-ukb-us-east-1/sumstats_flat_files/${filename}.tsv.bgz;done)
